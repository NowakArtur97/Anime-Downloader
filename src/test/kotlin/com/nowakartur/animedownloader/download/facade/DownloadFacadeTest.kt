...
class DownloadFacadeTest : SeleniumTest() {
    ...
    fun `when get download info should return correct download info`() {
        ...
        val title = "Tomodachi Game"
        val expectedDownloadInfo = listOf(
            DownloadInfo(title, XStreamCdnPage, 314.35f, "https://fembed-hd.com/f/-867japek5e34zj"),
            DownloadInfo(title, Mp4UploadPage, 314.3f, "http://www.mp4upload.com/tfhmno4rrqwa"),
            DownloadInfo(title, StreamSbPage, 220.2f, "https://sbplay2.xyz/d/wok4s5ql6ie6"),
        )

        val actualDownloadInfo = DownloadFacade.getDownloadInfo(title, webDriver, goloadUrl)
    ...
    fun `when download file in best quality should start downloading file from Mp4Upload`() {
        ...
        DownloadFacade.downloadInBestQuality(webDriver, DownloadInfo("Ao Ashi", Mp4UploadPage, 259.0f, mp4uploadUrl))
